...
class ControllerAgooLatestLatest extends Controller
{
	...
	public function __construct($registry) {
		...
		if (SC_VERSION > 23) {
			...
			$this->module_files = Array('agootemplates/blog/blog'.$ext,
										'agootemplates/record/record'.$ext);
	}
	...
	public function index($data) {
		...
		if (SC_VERSION > 23) {
			$this->data['latest_template'] = 'agoo/latest/latest';
		} else {
...
$this->language->get('entry_anchor_templates_clear') => ""
	...
	public function settings($data) {
		...
		$this->data['url_secure'] = array(0 => $this->language->get('url_secure_0'), 'http' => $this->language->get('url_secure_http'), 'https' => $this->language->get('url_secure_https') );

		if (!isset($this->data['ascp_settings']['seocms_url_secure'])) {
			$this->data['ascp_settings']['seocms_url_secure'] = false;
		}
		...
		if (SC_VERSION > 23) {
			$this->template = 'agoo/latest/settings';
		} else {
	...
	public function widget_services($data) {
		...
		if (SC_VERSION > 23) {
			$this->template = 'agoo/latest/widget_services';
		} else {
	...
	public function adapter_button($data) {
		...
		foreach ($this->data['adapter'] as $id => $adapter) {
			...
			$action               = array(
				...
				'href' => $this->url->link('catalog/adapter/update', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&widget=latest&adapter_theme=' . $adapter['adapter_name'] )
			);
	...
	public function adapter_form($data) {
        ...
        if (isset($this->request->get['widget']) && $this->request->get['widget'] == 'latest') {
	...
	public function adapter_update($data) {
   	...
   	public function run($data) {
		...
		if (!isset($this->request->get['source']) || $this->request->get['source'] == '') {
			if (SC_VERSION > 23) {
				$this->data['replace_main_name'][$this->data['theme']] = '{{ description }}';
			} else {
		...
		$this->data['file_theme'][$this->data['theme']]     = 'information/information' . $ext;

		if (isset($this->request->get['source']) && $this->request->get['source'] != '') {
			$this->data['file_theme'][$this->data['theme']] = 'common/success' . $ext;;
		}

		$this->data['file_theme']['shoppica2']         = 'information/information' . $ext;;
		$this->data['file_theme']['shoppica']          = 'information/information' . $ext;;
		...
		if (file_exists(DIR_CATALOG . 'view/theme/' . $this->data['theme'] . '/template/' . $this->data['file_theme'])) {
			...
			$this->data['file_theme'] = 'common/success' . $ext;;
			if (file_exists(DIR_CATALOG . 'view/theme/' . $this->data['theme'] . '/template/' . $this->data['file_theme'])) {
				$success_file = DIR_CATALOG . 'view/theme/' . $this->data['theme'] . '/template/' . $this->data['file_theme'];
				$success      = $this->data['success_data'] = file_get_contents($success_file);
			} else {
		...
		if (SC_VERSION > 23) {
			$this->data['template']          = 'catalog/adapter_form';
		} else {
			$this->data['template']          = 'catalog/adapter_form.tpl';
		}

		if (isset($this->request->get['adapter_theme'])) {
			$this->data['action'] = $this->url->link('catalog/adapter/update', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&widget=latest&adapter_theme=' . $this->request->get['adapter_theme']);
		} else {
			$this->data['action'] = $this->url->link('catalog/adapter/update', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&widget=latest&adapter_id=' . $this->request->get['adapter_id'] . $url);
		}
...
CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "blog_description` (
  ...
  `meta_description` varchar(320) COLLATE utf8_general_ci NOT NULL,
...
CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "record_related` (
  ...
  `pointer` varchar(128) COLLATE utf8_general_ci DEFAULT NULL,
...
";
        ...
        $brand_id = $this->layout_module('product/manufacturer/info', $this->language->get('text_brand'));
		...
		$r = $this->db->query("DESCRIBE " . DB_PREFIX . "record_description `meta_description`");
		if ($r->num_rows > 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_description` CHANGE `meta_description` `meta_description` varchar(320) COLLATE utf8_general_ci NOT NULL ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("DESCRIBE " . DB_PREFIX . "blog_description `meta_description`");
		if ($r->num_rows > 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_description` CHANGE `meta_description` `meta_description` varchar(320) COLLATE utf8_general_ci NOT NULL ";
			$query = $this->db->query($msql);
		}
		...
		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_related` WHERE Key_name = 'pointer'");
		if ($r->num_rows != 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_related` DROP INDEX pointer";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_related` WHERE Key_name = 'related'");
		if ($r->num_rows != 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_related` DROP INDEX related";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_related`  WHERE Key_name = 'related_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_related` ADD INDEX `related_id` (`related_id`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_related`  WHERE Key_name = 'pointer_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_related` ADD INDEX `pointer_id` (`pointer_id`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_related`  WHERE Key_name = 'pointer'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_related` ADD INDEX `pointer` (`pointer`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("DESCRIBE " . DB_PREFIX . "record_related `sc_id`");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_related` ADD `sc_id` INT(11) NOT NULL AUTO_INCREMENT FIRST, ADD UNIQUE `sc_id` (`sc_id`);";
			$query = $this->db->query($msql);
		}


		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog` WHERE Key_name ='status' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog`  ADD INDEX `status` (`status`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog` WHERE Key_name = 'sort_order' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog`  ADD INDEX `sort_order` (`sort_order`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("DESCRIBE `" . DB_PREFIX . "blog` `path`");

		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog` ADD `path` VARCHAR( 255 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `blog_id`";
			$query = $this->db->query($msql);
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog` ADD INDEX `path` (`path`) ";
			$query = $this->db->query($msql);
		}
		...
		$r = $this->db->query("DESCRIBE `" . DB_PREFIX . "url_alias_blog` `blog_id`");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "url_alias_blog` ADD `blog_id` INT(11) NOT NULL AFTER `query` ";
			$query = $this->db->query($msql);
			$msql = "ALTER TABLE `" . DB_PREFIX . "url_alias_blog` ADD INDEX `blog_id` (`blog_id`) ";
			$query = $this->db->query($msql);
			...
			if (!empty($query->rows)) {
				foreach ($query->rows as $row) {
					if (isset($row['blog_id']) && $row['blog_id']) {
                    	$this->db->query("
                    	UPDATE " . DB_PREFIX . "url_alias_blog
                    	SET blog_id = '" . (int) $row['blog_id'] . "'
                    	WHERE query = 'blog_id=" . (int) $row['blog_id'] . "' ");
			...
			$query = $this->db->query("SELECT record_id, blog_main FROM " . DB_PREFIX . "record");

			if (!empty($query->rows)) {
				foreach ($query->rows as $row) {
					if (isset($row['blog_main']) && $row['blog_main']) {
                    	$this->db->query("
                    	UPDATE " . DB_PREFIX . "url_alias_blog
                    	SET blog_id = '" . (int) $row['blog_main'] . "'
                    	WHERE query = 'record_id=" . (int) $row['record_id'] . "' ");
					}
				}
			}
		...
		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_to_layout` WHERE Key_name ='blog_id' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_to_layout`  ADD INDEX `blog_id` ( `blog_id` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_to_layout` WHERE Key_name ='store_id' ");
		if ($r->num_rows == 0) {

			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_to_layout`  ADD INDEX `store_id` ( `store_id` ) ";
			$query = $this->db->query($msql);

		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_to_layout` WHERE Key_name ='layout_id' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_to_layout`  ADD INDEX `layout_id` ( `layout_id` ) ";
			$query = $this->db->query($msql);
		}


		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_description` WHERE Key_name ='blog_id' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_description`  ADD INDEX `blog_id` ( `blog_id` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_description` WHERE Key_name ='language_id' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_description`  ADD INDEX `language_id` ( `language_id` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_to_store` WHERE Key_name ='blog_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_to_store`  ADD INDEX `blog_id` ( `blog_id`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "blog_to_store` WHERE Key_name ='store_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "blog_to_store`  ADD INDEX `store_id` ( `store_id`  ) ";
			$query = $this->db->query($msql);
		}


		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_to_blog` WHERE Key_name ='record_id' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_to_blog`  ADD INDEX `record_id` ( `record_id` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_to_blog` WHERE Key_name ='blog_id' ");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_to_blog`  ADD INDEX `blog_id` ( `blog_id` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record` WHERE Key_name ='date_available'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record`  ADD INDEX `date_available` ( `date_available`  ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record` WHERE Key_name ='date_end'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record`  ADD INDEX `date_end` ( `date_end` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record` WHERE Key_name ='status'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record`  ADD INDEX `status` ( `status` ) ";
			$query = $this->db->query($msql);

		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record` WHERE Key_name ='sort_order'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record`  ADD INDEX `sort_order` ( `sort_order` ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_to_store` WHERE Key_name ='record_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_to_store`  ADD INDEX `record_id` ( `record_id`) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_to_store` WHERE Key_name ='store_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_to_store`  ADD INDEX `store_id` ( `store_id`  ) ";
			$query = $this->db->query($msql);
		}


		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_description` WHERE Key_name ='record_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_description`  ADD INDEX `record_id` ( `record_id`  ) ";
			$query = $this->db->query($msql);
		}

		$r = $this->db->query("SHOW KEYS FROM `" . DB_PREFIX . "record_description` WHERE Key_name ='language_id'");
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "record_description`  ADD INDEX `language_id` ( `language_id`  ) ";
			$query = $this->db->query($msql);
		}
