...
class ControllerAgooRelatedRelated extends Controller
{
	...
	public function index($data) {
		...
		if (SC_VERSION > 23) {
			$this->data['related_template'] = 'agoo/latest/latest';
		} else {
...
$this->language->get('entry_anchor_templates_clear') => ""
	...
	public function settings($data)	{
		...
		if (!isset($this->data['ascp_settings']['related_theme'])) {
			$this->data['ascp_settings']['related_theme'] = false;
		}

		if (SC_VERSION > 23) {
			$this->template = 'agoo/related/settings';
		} else {
