...
class ControllerExtensionModuleTextDeliveryProduct extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting($this->prefix . 'text_delivery_product', $this->request->post);

            $this->session->data['success'] = $this->language->get('text_success');

            if (isset($this->request->post['apply'])) {
                $this->response->redirect($this->url->link('extension/module/text_delivery_product', $this->token_var . '=' . $this->session->data[$this->token_var], true));
            } else {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $heading_title,
            'href' => $this->url->link('extension/module/text_delivery_product', $this->token_var . '=' . $this->session->data[$this->token_var], true)
        );

        $data['prefix'] = $this->prefix;
        $data['token_var'] = $this->token_var;
        $data[$this->token_var] = $this->session->data[$this->token_var];
        $data['action'] = $this->url->link('extension/module/text_delivery_product', $this->token_var . '=' . $this->session->data[$this->token_var], true);
        ...
        if (isset($this->request->post[$this->prefix . 'text_delivery_product_status'])) {
            $data[$this->prefix . 'text_delivery_product_status'] = $this->request->post[$this->prefix . 'text_delivery_product_status'];
        } else {
            $data[$this->prefix . 'text_delivery_product_status'] = $this->config->get($this->prefix . 'text_delivery_product_status');
        }
        ...
        $this->response->setOutput($this->load->view('extension/module/text_delivery_product', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/text_delivery_product')) {
