...
class ControllerExtensionModuleTopSalesHome extends Controller {
    ...
    public function index() {
        $data = $this->load->language('extension/module/top_sales_home');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting($this->prefix . 'top_sales_home', $this->request->post);

            $this->session->data['success'] = $this->language->get('text_success');

            if (isset($this->request->post['apply'])) {
                $this->response->redirect($this->url->link('extension/module/top_sales_home', $this->token_var . '=' . $this->session->data[$this->token_var], true));
            } else {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $heading_title,
            'href' => $this->url->link('extension/module/top_sales_home', $this->token_var . '=' . $this->session->data[$this->token_var], true)
        );

        $data['prefix'] = $this->prefix;
        $data['token_var'] = $this->token_var;
        $data[$this->token_var] = $this->session->data[$this->token_var];
        $data['action'] = $this->url->link('extension/module/top_sales_home', $this->token_var . '=' . $this->session->data[$this->token_var], true);
        ...
        if (isset($this->request->post[$this->prefix . 'top_sales_home_status'])) {
            $data[$this->prefix . 'top_sales_home_status'] = $this->request->post[$this->prefix . 'top_sales_home_status'];
        } else {
            $data[$this->prefix . 'top_sales_home_status'] = $this->config->get($this->prefix . 'top_sales_home_status');
        }
        ...
        $this->response->setOutput($this->load->view('extension/module/top_sales_home', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/top_sales_home')) {
