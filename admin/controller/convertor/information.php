...
class ControllerConvertorInformation extends Controller
{
	...
	private function loadinformation($information_id)
	{
		...
		if ($this->table_exists(DB_PREFIX . "information_product")) {
		      $query_product = $this->db->query("SELECT *
		      FROM " . DB_PREFIX . "information_product n
		      ");
         		foreach ($query_product->rows as $num => $information) {

	         		$information_product[] = array('information_id' => $information['information_id'], 'product_id' => $information['product_id']);
         		}
		}
      ...
      foreach ($query->rows as $num => $information) {
      			 $my_information['record_id'] = $information['information_id'];
              ...
              if (strlen($information['date_added']) < 19) {
              	$my_information['date_added'] = $information['date_added'] . ' 00:00:00';
              }
              $my_information['date_end'] = '2030-11-11 00:00:00';
              $my_information['date_available'] = $information['date_added'];
              $my_information['date_added'] = $information['date_added'];
              $my_information['date_modified'] = $information['date_added'];
              if (!empty($information_product)) {
         		foreach ($information_product as $num  => $ip_information_id) {
                	if ($ip_information_id['information_id'] == $information['information_id']) {
                		$my_information['product_related'][$ip_information_id['product_id']] = $ip_information_id['product_id'];
                	}
         		}
         		}


              $my_information['record_tag'] = Array();
	 ...
	 foreach ($this->data['information_store'] as $number =>  $n_s) {
		if (isset($n_s['store_id'])) {
			$my_information['record_store'][$number] = $n_s['store_id'];
		} else {
			$my_information['record_store'][$number] = 0;
		}
     }
