...
class ModelCatalogFields extends Model
{
	public function getFields($data = array()) {
		if ($data['mark'] == 'product_id' || $data['mark'] == 'record_id') {
			$sql   = "SELECT *
	...
	public function getFieldByName($field_name) {
		...
		FROM " . DB_PREFIX . "fields f
		LEFT JOIN " . DB_PREFIX . "fields_description fd ON (f.field_id = fd.field_id)
		WHERE f.field_name = '" .  $this->db->escape(preg_replace('/[^a-zA-Z0-9\_\s+]/', '', html_entity_decode($field_name, ENT_QUOTES, 'UTF-8')))  . "' AND fd.language_id = '" . (int) $this->config->get('config_language_id') . "'");
		return $query->row;
	}
	...
	public function table_exists($tableName) {
		$found= false;
		$tableName = $this->db->escape(preg_replace('/[^a-zA-Z0-9\_\s+]/', '', html_entity_decode($tableName, ENT_QUOTES, 'UTF-8')));
	...
	public function editField($field_id, $data) {
        ...
        $field_id = (int) $field_id;
        $data['field_name'] = $this->db->escape(preg_replace('/[^a-zA-Z0-9\_\s+]/', '', html_entity_decode($data['field_name'], ENT_QUOTES, 'UTF-8')));
		...
		if ($this->db->escape($data['field_type']) == 'select') {
          	$type_field = "TEXT";
          	$type_filed_end = " COLLATE utf8_general_ci NOT NULL";
		}

		if ($this->db->escape($data['field_type']) == 'rating') {
			$type_field = "INT";
		...
		if ($r->num_rows == 0) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "review_fields` ADD `" . $this->db->escape($data['field_name']) . "` text COLLATE utf8_general_ci NOT NULL";
			$query = $this->db->query($msql);
		}

       	$data_info = $this->getFieldOnly($field_id);
		$data_info['field_name'] = $this->db->escape(preg_replace('/[^a-zA-Z0-9\_\s+]/', '', html_entity_decode($data_info['field_name'], ENT_QUOTES, 'UTF-8')));

		if ($data['field_name']!= $data_info['field_name']) {
         $msql  = "ALTER TABLE `" . DB_PREFIX . "review_fields` CHANGE `".$this->db->escape($data_info['field_name'])."` `".$this->db->escape($data['field_name'])."`" . $type_field . $type_filed_end;
			$query = $this->db->query($msql);
		}
		...
		foreach ($data['fields_description'] as $language_id => $value) {
            ...
            field		= '" . base64_encode(serialize($value['field'])) . "'"
			);
		}
	...
	public function addField($data) {
		...
		if ($r->num_rows == 0) {
	        ...
	        $this->request->get['field_id'] = (int)$data['field_id'];
	        $this->editField((int)$data['field_id'], $data);
        } else {
          	$error = true;
        }
