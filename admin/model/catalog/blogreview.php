...
class ModelCatalogBlogreview extends Model
{
	public function editReview($review_id, $data)
	{
		if ($review_id != "" && $data['date_added'] != "") {
			$this->db->query("UPDATE " . DB_PREFIX . "review SET   date_added = '" . $this->db->escape($data['date_added']) . "' WHERE review_id = '" . (int) $review_id . "'");
				...
				if ($sql_add != "") {
					 ...
					 $sql   = "SELECT * FROM  " . DB_PREFIX . "review_fields rf
                      ...
                      if (count($query->rows) > 0) {
						$sql = "UPDATE " . DB_PREFIX . "review_fields SET " . $sql_add . " WHERE review_id='" . (int) $review_id . "' AND mark='".$this->db->escape($data['mark'])."' ";
