...
class ModelCatalogBlogComment extends Model
{
	public function addComment($data)
	{
		...
		if (isset($data['date_available']) && $data['date_available'] != '') {
			$sql_date_available = "date_added = '".$this->db->escape($data['date_available'])."', ";
		} else {
		...
		if (isset($data['af'])) {
			...
			if ($af_bool) {
				...
				if ($sql_add != "") {
					$sql = "INSERT INTO " . DB_PREFIX . "review_fields SET " . $sql_add . " review_id='" . (int) $review_id . "', mark='".$this->db->escape($data['mark'])."' ";
					$this->db->query($sql);
				}
			}
		}
	...
	public function editComment($comment_id, $data)
	{
        ...
        if (!isset($data['cmswidget']))  $data['cmswidget'] = false;
		...
		WHERE ".$this->db->escape($data['mark_name'])."_id = '" . (int) $comment_id . "'");
			   ...
			   $i=1;
				...
				if ($sql_add != "") {
					 ...
					 $sql   = "SELECT * FROM  " . DB_PREFIX . "review_fields rf
                      ...
                      if (count($query->rows) > 0) {
						$sql = "UPDATE " . DB_PREFIX . "review_fields SET " . $sql_add . " WHERE review_id='" . (int) $comment_id . "' AND mark='".$this->db->escape($data['mark'])."' ";

                      } else {
			  		$sql = "INSERT INTO " . DB_PREFIX . "review_fields SET " . $sql_add . ", review_id='" . (int) $comment_id . "', mark='".$this->db->escape($data['mark'])."' ";
	...
	public function deleteComment($comment_id, $mark_name = 'comment')
	{
        ...
        foreach ($comments_data as $num => $comment_id) {
		 $this->db->query("DELETE FROM `" . DB_PREFIX . $this->db->escape($mark_name). "` WHERE ".$this->db->escape($mark_name)."_id = '" . (int) $comment_id . "'");
		 $this->db->query("DELETE FROM `" . DB_PREFIX . "rate_".$this->db->escape($mark_name)."` WHERE ".$this->db->escape($mark_name)."_id = '" . (int) $comment_id . "'");
		 $this->db->query("DELETE FROM `" . DB_PREFIX . "agoo_signer` WHERE id = '" . (int) $comment_id . "' AND pointer='".$this->db->escape($mark_name)."_id'");
        }
	...
	public function getCommentsWithChild($comment_id, $mark_name = 'comment')
	{
			...
			$sql  = "SELECT r.".$this->db->escape($mark_name)."_id as id, r.parent_id as parent_id
			FROM " . DB_PREFIX . $this->db->escape($mark_name)." r
			WHERE
	...
	public function getComment($comment_id, $mark_name = 'comment')
	{
		...
		FROM " . DB_PREFIX . $this->db->escape($mark_name)." r
		LEFT JOIN " . DB_PREFIX . "agoo_signer agoos ON (agoos.pointer = '".$this->db->escape($mark_name)."_id' AND agoos.id = r.".$this->db->escape($mark_name)."_id)
		WHERE
		r.".$this->db->escape($mark_name)."_id = '" . (int) $comment_id . "'";
	...
	public function getComments($data = array()) {
		...
		$sql  = "SELECT SQL_CALC_FOUND_ROWS r.".$mark."_id, r.".$this->db->escape($data['mark']).", r.cmswidget, r.rating_mark, r.text, pd.name, r.type_id, r.author, r.rating, r.status, r.date_added, r.language_id
	...
	public function getRecordIdbyCommentId($comment_id, $mark = 'comment')
	{
		...
		$sql   = "SELECT DISTINCT * FROM " . DB_PREFIX . $this->db->escape($mark)." r WHERE r.".$this->db->escape($mark)."_id = '" . (int) $comment_id . "'";
		$query = $this->db->query($sql);
	...
	{
		$sql = "DELETE FROM `" . DB_PREFIX . "rate_".$this->db->escape($mark_name)."` WHERE rate_id = '" . (int) $id . "'";
		$this->db->query($sql);
	...
	public function switchstatus($id, $mark_name = 'comment')
	{
		...
		$sql = "UPDATE `" . DB_PREFIX .$this->db->escape($mark_name)."` SET `status` = abs(`status` - 1) WHERE ".$this->db->escape($mark_name)."_id = '".(int)$id."'";

		$this->db->query($sql);

		$sql = "SELECT status FROM `" . DB_PREFIX .$this->db->escape($mark_name)."` WHERE ".$this->db->escape($mark_name)."_id = '" . (int) $id . "'";
