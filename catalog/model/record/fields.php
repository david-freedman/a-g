...
// Commercial license
class ModelRecordFields extends Model
{
	...
	public function editField($field_id, $data) {
		...
		$r = $this->db->query("DESCRIBE " . DB_PREFIX . "review_fields `" . $data['field_name'] . "`");

		if ($r->num_rows > 0 && strtoupper($r->row['Type']) != strtoupper ($type_field)) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "review_fields` CHANGE `" . $data['field_name'] . "` `" . $data['field_name'] . "`" .$type_field . $type_filed_end;
			$query = $this->db->query($msql);
		}

       $data_info = $this->getFieldOnly((int)$field_id);
       ...
       $data_info['field_name'] = $this->db->escape(preg_replace('/[^a-zA-Z0-9\_\s+]/', '', html_entity_decode($data_info['field_name'], ENT_QUOTES, 'UTF-8')));
		...
		if ($data['field_name'] != $data_info['field_name']) {
			$msql = "ALTER TABLE `" . DB_PREFIX . "review_fields` CHANGE `" . $data_info['field_name'] . "` `" . $data['field_name'] . "`" .$type_field . $type_filed_end;
			$query = $this->db->query($msql);
		}
