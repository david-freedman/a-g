...
<?php } ?>
	...
	?>
	<div id="commentlink_<?php  echo $comment['comment_id']; ?>_<?php  echo $cmswidget; ?>" class="<?php echo $prefix;?>form_customer_pointer comment_content level_<?php  echo $comment['level']; ?>" <?php if (isset($settings_widget['micro_status']) && $settings_widget['micro_status']) { ?>itemprop="review" itemscope itemtype="http://schema.org/Review"<?php } ?>>
		...
		<div class="padding2 comment_block" style="<?php echo $back_color; ?>">
			...
			<span <?php if (isset($settings_widget['micro_status']) && $settings_widget['micro_status']) { ?>itemprop="author"<?php } ?>><?php  echo $comment['author']; ?></span>
			...
			<div class="sc_h_s">
					...
					<img class="sc-border-none"  title="<?php echo $comment['rating']; ?>" alt="<?php echo $comment['rating']; ?>" src="catalog/view/theme/<?php echo $theme_stars; ?>/image/blogstars-<?php echo $comment['rating']; ?>.png">
					<?php } ?>
 			</div>
  			<?php } ?>
			...
			<time <?php if (isset($settings_widget['micro_status']) && $settings_widget['micro_status']) { ?>itemprop="datePublished"<?php } ?> datetime="<?php echo $comment['date_added_iso']; ?>"><?php echo $comment['date_added']; ?></time><?php } ?>&nbsp;&nbsp;<a href="<?php echo $url;?>#commentlink_<?php  echo $comment['comment_id']; ?>_<?php  echo $cmswidget; ?>"><?php if (SC_VERSION > 15) { ?><i class="fa fa-link" aria-hidden="true"></i><?php } else { ?>#<?php } ?></a>
...
<?php include(realpath(dirname(__FILE__)).'/../fields_view.tpl'); ?>
			...
			<div class="voting sc_h_s <?php  if ($comment['customer_delta'] < 0) echo 'voted_blog_minus';  if ($comment['customer_delta'] > 0) echo 'voted_blog_plus';?> floatright margintop5"  id="voting_<?php  echo $comment['comment_id']; ?>">
				<!-- <noindex> -->
				<?php
