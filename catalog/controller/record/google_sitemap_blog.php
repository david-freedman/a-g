...
// Commercial license
if (!class_exists('ControllerRecordGoogleSitemapBlog', false)) {
...
//			$this->google_sitemap_blog_language_status = true;
		...
		if (!$this->config->get('config_image_popup_width') || $this->config->get('config_image_popup_width')=='') {
			...
			if (!$this->config->get($this->config->get('config_theme') . '_image_popup_width')) {
        		...
        		$this->config->set('config_image_popup_width', $this->config->get($this->config->get('config_theme') . '_image_popup_width'));
        	}
		}
		if (!$this->config->get('config_image_popup_height') || $this->config->get('config_image_popup_height')=='') {

			if (!$this->config->get($this->config->get('config_theme') . '_image_popup_height')) {
        		$this->config->set('config_image_popup_height', '220');
        	}  else {
        		$this->config->set('config_image_popup_height', $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
        	}
		}
	...
	public function setSitemap($output) {
		if ($this->config->get('google_sitemap_blog_status')) {
			...
			if (isset($this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_status']) && $this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_status']) {
				...
				if (isset($this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_route']) && $this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_route'] != '' && $this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_route'] == $data['route'] && $data['route'] != 'record/google_sitemap_blog') {
					...
					if (isset($this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_tag']) && $this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_tag'] != '') {
						$google_sitemap_blog_inter_tag = html_entity_decode($this->data['ascp_settings_sitemap']['google_sitemap_blog_inter_tag'], ENT_QUOTES, 'UTF-8');
						if (strpos($output, $google_sitemap_blog_inter_tag) === false) {
						} else {

								$sitemap_html = $this->getascp();
	...
	public function get_sitemap_content() {
		if ($this->config->get('google_sitemap_blog_status')) {
			...
			if ((isset($this->data['settings_general']['seocms_url_secure']) && $this->data['settings_general']['seocms_url_secure'] == 'https' && $this->data['settings_general']['seocms_url_secure'] != 'http') || ((isset($_SERVER['HTTPS']) && (strtolower($_SERVER['HTTPS']) == 'on' || $_SERVER['HTTPS'] == '1')) || (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && (strtolower($_SERVER['HTTP_X_FORWARDED_PROTO']) == 'https') || (!empty($_SERVER['HTTP_X_FORWARDED_SSL']) && strtolower($_SERVER['HTTP_X_FORWARDED_SSL']) == 'on'))) ) {
            	$domen = $this->config->get('config_ssl');
            } else {
            	$domen = $this->config->get('config_url');
            }
            /*
			...
			*/
                $url = str_replace('&', '&amp;', str_replace('&amp;', '&', $this->url->link('common/home')));
...
if (!class_exists('sitemapCache', false)) {
class sitemapCache
{
