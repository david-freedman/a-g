...
class ControllerAgooBlogsBlogs extends Controller
{
	...
	public function index($data) {
		$time_visual['start'] = microtime(true);
		$this->data = $data;
					...
					if ($blog_info) {
						if ($blog_info['image']) {
							if (isset($this->data['settings']['avatar']['width']) && isset($this->data['settings']['avatar']['height']) && $this->data['settings']['avatar']['width'] != "" && $this->data['settings']['avatar']['height'] != "") {
								$thumb = $this->seocmslib->resizeme($blog_info['image'], $this->data['settings']['avatar']['width'], $this->data['settings']['avatar']['height'], $this->data['settings']['image_adaptive_status']);
							} else {
								$thumb = $this->seocmslib->resizeme($blog_info['image'], 150, 150, $this->data['settings']['image_adaptive_status']);
							}
						} else {
							$thumb = '';
						}
		...
		if (isset($this->data['blogs']) && count($this->data['blogs']) > 0) {
			...
			if (isset($this->data['settings']['order_ad']) && $this->data['settings']['order_ad'] == 'asc') {
				$order_ad_sorting = 'compareblogs';
			} else {
				$order_ad_sorting = 'compareblogsdesc';
			}

			$this->data['categories_blogs'] = my_sort_div_blogs($new_arr, 0, -1, $order_ad_sorting);
        ...
        $this->data['template'] = 'agootemplates/widgets/blogs/' . $template;
...
if (!function_exists('my_sort_div_blogs')) {
	function my_sort_div_blogs($data, $parent = 0, $lev = -1, $sorting = 'compareblogs') {
		$arr = $data[$parent];
		...
		for ($i = 0; $i < count($arr); $i++) {
			...
			if (isset($data[$arr[$i]['blog_id']])) {
				$m = my_sort_div_blogs($data, $arr[$i]['blog_id'], $lev, $sorting);
				$z = array_merge($z, $m);
			}
